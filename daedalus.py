...
# number of times to adjust the constsant with binary search
...
def process_yolo_output(out, anchors, mask):
    ...
    box_confidence = tf.sigmoid(
        out[..., 4:5], name='objectness')  # (N, S, S, 3, 1)
    box_class_probs = tf.sigmoid(
        out[..., 5:], name='class_probs')  # (N, S, S, 3, 80)

    # Dimension of the used three anchor boxes [[x,x], [x,x], [x,x]].
    anchors = np.array([anchors[i] for i in mask])
    # duplicate to shape (batch, height, width, num_anchors, box_params).
    anchors = np.repeat(anchors[np.newaxis, :, :],
                        grid_w, axis=0)          # (S, 3, 2)
    anchors = np.repeat(anchors[np.newaxis, :, :, :],
                        grid_h, axis=0)       # (S, S, 3, 2)
    anchors = np.repeat(anchors[np.newaxis, :, :, :, :],
                        batchsize, axis=0)  # (N, S, S, 3, 2)
    anchors_tensors = tf.constant(
        anchors, dtype=tf.float32, name='anchor_tensors')

    box_xy = tf.sigmoid(out[..., 0:2], name='box_xy')  # (N, S, S, 3, 2)
    box_wh = tf.identity(
        tf.exp(out[..., 2:4]) * anchors_tensors, name='box_wh')  # (N, S, S, 3, 2)
    ...
    boxes = tf.reshape(boxes, [batchsize, -1, boxes.shape[-2],
                       boxes.shape[-1]], name='boxes')  # (N, S*S, 3, 4)
...
def expectation_of_IoUs_accross_classes(boxes, box_scores):
    ...
    others, dominating_boxes = tf.dynamic_partition(
        boxes, index, num_partitions=2, name='dynamic_partition')
...
class Daedalus:
    ...
    def __init__(self, sess, model, target_class=ATTACK_CLASS, attack_mode=ATTACK_MODE, img_shape=IMAGE_SHAPE,
        ...
        def select_class(target_class, boxes, objectness, box_scores, mode='all'):
            box_classes = tf.cast(
                tf.argmax(box_scores, axis=-1), tf.int32, name='box_classes')
            class_counts = tf.bincount(box_classes)
            print(class_counts)
            if mode == 'all':
                selected_boxes = tf.reshape(boxes, [BATCH_SIZE, -1, 4])
                selected_scores = tf.reshape(
                    box_scores, [BATCH_SIZE, -1, CLASS_NUM])
                if objectness == None:
                    return selected_boxes, None, selected_scores
                selected_objectness = tf.reshape(
                    objectness, [BATCH_SIZE, -1, 1])
                return selected_boxes, selected_objectness, selected_scores
            elif mode == 'most':
                selected_cls = tf.argmax(class_counts)
            elif mode == 'least':
                class_counts = tf.where(tf.equal(class_counts, 0), int(
                    1e6)*tf.ones_like(class_counts, dtype=tf.int32), class_counts)
            ...
            index = tf.cast(index, tf.int32)
            _, selected_boxes = tf.dynamic_partition(
                boxes, index, num_partitions=2, name='dynamic_partition')
            _, selected_scores = tf.dynamic_partition(
                box_scores, index, num_partitions=2, name='dynamic_partition')
            selected_boxes = tf.reshape(selected_boxes, [BATCH_SIZE, -1, 4])
            selected_scores = tf.reshape(
                selected_scores, [BATCH_SIZE, -1, CLASS_NUM])
            if objectness == None:
                return selected_boxes, None, selected_scores
            _, selected_objectness = tf.dynamic_partition(
                objectness, index, num_partitions=2, name='dynamic_partition')
            selected_objectness = tf.reshape(
                selected_objectness, [BATCH_SIZE, -1, 1])
            return selected_boxes, selected_objectness, selected_scores
        ...
        self.consts = tf.Variable(
            np.zeros(batch_size), dtype=tf.float32, name='self.consts')
        ...
        self.newimgs = tf.tanh(perturbations + self.timgs) * \
        ...
        boxes, objectness, classprobs = select_class(
            self.target_class, boxes, objectness, classprobs, mode=self.attack_mode)
        ...
        self.box_scores = tf.multiply(self.obj_scores, tf.reduce_max(
            self.class_probs, axis=-1, keepdims=True))

        # Optimisation metrics:
        self.l2dist = tf.reduce_sum(tf.square(
            self.newimgs - (tf.tanh(self.timgs) * self.boxmul + self.boxplus)), [1, 2, 3])

        # Define DDoS losses: loss must be a tensor here!
        # Make the objectness of all detections to be 1.
        self.loss1_1_x = tf.reduce_mean(
            tf.square(self.box_scores - 1), [-2, -1])

        # Minimising the size of all bounding box.
        # self.f1 = tf.reduce_mean(IoU_expts)
        self.f3 = tf.reduce_mean(
            tf.square(tf.multiply(self.bw, self.bh)), [-2, -1])
        ...
        self.init = tf.variables_initializer(
            var_list=[perturbations] + new_vars)
    ...
    def attack_batch(self, imgs):
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                _, l, l2s, l1s, nimgs, c = self.sess.run(
                    [self.train, self.loss, self.l2dist, self.loss_adv, self.newimgs, self.consts])
                # print out the losses every 10%
                if iteration % (self.MAX_ITERATIONS // 10) == 0:
                    print('===iteration:', iteration, '===')
                    print('attacked box number:', sess.run(self.bw).shape)
                    print('loss values of box confidence and dimension:',
                          sess.run([self.loss1_1_x, self.f3]))
    ...
    def attack(self, imgs):
        ...
        for i in range(0, len(imgs), self.batch_size):
            ...
            if not os.path.exists(path):
                os.makedirs(path)
            np.save(
                path+'/Distortions of images {0} to {1}.npy'.format(i, i+self.batch_size), dists)
            for j in range(len(X_adv)):
                io.imsave(path+'/Best example of {1} Distortion {2}.png'.format(
...
if __name__ == '__main__':
    ...
    np.savez(SAVE_PATH+'{0} confidence/Daedalus example batch.npz'.format(
        CONFIDENCE), X_adv=X_adv, distortions=distortions)
