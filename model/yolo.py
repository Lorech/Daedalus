...
class YOLO:
    def __init__(self, obj_threshold, nms_threshold):
        ...
        self._yolo = load_model('./model/yolo.h5')
    ...
    def _process_feats(self, out, anchors, mask):
        ...
        # print('box confidences are: {0}',format(box_confidence))
    ...
    def _soft_nms(self, boxes, scores):
        ...
        while order.size > 0:
            ...
            xx1 = np.maximum(x[i], x[order])
            yy1 = np.maximum(y[i], y[order])
            xx2 = np.minimum(x[i] + w[i], x[order] + w[order])
            yy2 = np.minimum(y[i] + h[i], y[order] + h[order])
            ...
            # change scores based on IoU
            scores[order] *= np.exp((ovr**2)/(-0.5))
            scores = scores[order]
            order = scores.argsort()[::-1]  # re-order
    ...
    def _yolo_out(self, outs, shape):
        ...
        for c in set(classes):
            ...
            keep = self._nms_boxes(b, s)  # use NMS
            # keep = self._soft_nms(b, s) #use soft-NMS
    ...
    def predict(self, image, shape):
        ...
        raw_outs = self._yolo.predict(image)
        boxes, classes, scores = self._yolo_out(raw_outs, shape)

        return boxes, classes, scores, raw_outs
